...
heapnode* mergeHeapNode(heapnode* h1, heapnode* h2){
    if (h1->priority > h2->priority){
        h1->degree += 1;
        heapnode** temp = (heapnode**) realloc(h1->child,(h1->degree)*sizeof(heapnode*));
        h1->child = temp;
        h1->child[h1->degree-1] = h2;
        
        return h1;
    }else{
        h2->degree += 1;
        heapnode** temp = (heapnode**) realloc(h2->child,(h2->degree)*sizeof(heapnode*));
        h2->child = temp;
        h2->child[h2->degree-1] = h1;
