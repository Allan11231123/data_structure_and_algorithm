...
void attack(int label, classmateNode* power[], int N, int position[],int reward){
    long long gain = N-1;
    for(int i=0;i<N;i++){
...
//     }
// }
void lastRankPower(classmateNode* power[],long long value, int N, int reward){
...
int main(){
    ...
    for(int i=0;i<T;i++){
        ...
        case 1:
            scanf("%d",&label);
            attack(label,power,N,position,reward);
            reward = 0;
            // printAll(power,N);
            break;
        case 2:
